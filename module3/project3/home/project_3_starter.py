...
project_helper.print_dataframe(volume)
project_helper.print_dataframe(dividends)
...
(close*volume).div((close*volume).sum(axis=1), axis=0)
...
def generate_dollar_volume_weights(close, volume):
    ...
    dollar_volume_weights = (close*volume).div((close*volume).sum(axis=1), axis=0)
...
(np.cumsum(dividends)).div(np.cumsum(np.sum(dividends, axis=1)).shift(1), axis=0)
...
def calculate_dividend_weights(dividends):
    ...
    """
    #TODO: Implement function
    dividend_weights = (np.cumsum(dividends)).div(np.cumsum(np.sum(dividends, axis=1)), axis=0)
...
(1 + np.sum(returns, axis=1)).cumprod()
...
def calculate_cumulative_returns(returns):
    ...
    """
    #TODO: Implement function
    cumulative_returns = (1 + np.sum(returns, axis=1)).cumprod()
...
def get_covariance_returns(returns):
    ...
    """
    #TODO: Implement function
    returns_covariance = np.cov(returns.fillna(0).values, rowvar=False)
...
def get_optimal_weights(covariance_returns, index_weights, scale=2.0):
    ...
    portfolio_cov_quad = cvx.quad_form(x, covariance_returns)
    index_diff = cvx.norm(x - index_weights, p = 2)
    
    objective = cvx.Minimize(portfolio_cov_quad + scale * index_diff)
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    for i in range(chunk_size, len(returns), shift_size):
        ...
        cov_returns = get_covariance_returns(chunk)
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    num_rebalance_event_year = n_trading_days_in_year / shift_size
    portfolio_turnover = (sum_total_turnover / rebalance_count) * num_rebalance_event_year
